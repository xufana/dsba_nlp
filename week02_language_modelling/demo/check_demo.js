/* Check the built demo page in headless Chrome over the DevTools protocol.
   ...
   Run from week02_language_modelling/demo/ after build_demo.py:
       node check_demo.js                 # checks + screenshots into check_out/
       node check_demo.js week02_demo.html check_steps.json
...
const PAGE = path.resolve(process.argv[2] || "week02_demo.html");
...
(async () => {
  ...
  for (const st of STEPS.steps || []) {
    const t = Date.now();
    try {
      const r = await send("Runtime.evaluate", { expression: `(function(){ try { return String((function(){ ${st.expr} })()); } catch (e) { return "EXC " + e.message + " @ " + (e.stack || "").split("\\n")[1]; } })()`, returnByValue: true }, st.timeout || 20000);
